...
object DateTimeToEpoch {
    ...
    fun dateTimeToEpochLong(dateTime: String): Long {
        ...
        return SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(dateTime.toString()).time.toLong()

    }
    fun dateTimeToEpochFloat(dateTime: String): Float {
        //Log.d(TAG, "Passed $dateTime")
        //Log.d(TAG, "Returned $df")
        return SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(dateTime.toString()).time.toFloat()
